...
public class PricingService {
        ...
        if (secimler != null) {
            ...
            for(OzellikTipi tip : secimler.keySet()){
                ...
                for(String secim : secimler.get(tip)){
                    ...
                    // RAL için "Boyalı - XXXX" formatını "Boyalı" olarak eşleştir
                    String aramaSecim = secim;
                    if (tip == OzellikTipi.RAL && secim != null && secim.startsWith("Boyalı -")) {
                        aramaSecim = "Boyalı";
                    }

                    OzellikOran oran = oranMap.get(aramaSecim);
 ...
 public PricingResult fiyatHesapla(
     ...
     return new PricingResult(
             ...
             base.getOranEkleri(),
             motorToplam,
             yuvarlaBes(yeniToplam)
     );
 }

    private static double yuvarlaBes(double fiyat) {
        return Math.ceil(fiyat / 5.0) * 5.0;
    }
}
